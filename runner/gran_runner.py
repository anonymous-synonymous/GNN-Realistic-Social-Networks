...
class GranRunner(object):
  ...
  def train(self):
    ...
    if self.train_conf.optimizer == 'SGD':
      ...
      optimizer = optim.Adam(params, lr=self.train_conf.lr, weight_decay=self.train_conf.wd, capturable = True)     #Capturable added for PyTorch updates
    else:
        ...
        for ff in range(self.dataset_conf.num_fwd_pass):
          ...
          if self.use_gpu:
            for dd, gpu_id in enumerate(self.gpus):
              ...
              data['edges'] = batch_data[dd][ff]['edges'].to(self.device)
              data['node_idx_gnn'] = batch_data[dd][ff]['node_idx_gnn'].to(self.device)
              data['node_idx_feat'] = batch_data[dd][ff]['node_idx_feat'].to(self.device)
              data['label'] = batch_data[dd][ff]['label'].to(self.device)
              data['att_idx'] = batch_data[dd][ff]['att_idx'].to(self.device)
              data['subgraph_idx'] = batch_data[dd][ff]['subgraph_idx'].to(self.device)
              data['subgraph_idx_base'] = batch_data[dd][ff]['subgraph_idx_base'].to(self.device)
  ...
  def test(self):
    ...
    mmd_degree_dev, mmd_clustering_dev, mmd_4orbits_dev, mmd_spectral_dev, mmd_radius_dev, mmd_diffusion_dev_durs, mmd_diffusion_dev_sats = evaluate(self.graphs_dev[:n_gen], graphs_gen, degree_only=False)
    mmd_num_nodes_dev = compute_mmd([np.bincount(num_nodes_dev[:n_gen])], [np.bincount(num_nodes_gen)], kernel=gaussian)
    ...
    mmd_degree_test, mmd_clustering_test, mmd_4orbits_test, mmd_spectral_test, mmd_radius_test, mmd_diffusion_test_durs, mmd_diffusion_test_sats = evaluate(self.graphs_test[:n_gen], graphs_gen, degree_only=False)
    mmd_num_nodes_test = compute_mmd([np.bincount(num_nodes_test[:n_gen])], [np.bincount(num_nodes_gen)], kernel=gaussian)
    ...
    logger.info("\nValidation MMD scores of"
                ...
                "\ncommunity     {}".format(mmd_num_nodes_dev, mmd_degree_dev, mmd_clustering_dev, mmd_4orbits_dev,
                                            mmd_spectral_dev, mmd_diffusion_dev_durs, mmd_diffusion_dev_sats, mmd_radius_dev, mmd_num_coms_dev))
    ...
    logger.info("\nTest MMD scores of"
                ...
                "\ncommunity     {}".format(mmd_num_nodes_test, mmd_degree_test, mmd_clustering_test, mmd_4orbits_test,
                                            mmd_spectral_test, mmd_diffusion_test_durs, mmd_diffusion_test_sats, mmd_radius_test, mmd_num_coms_test))
  ...
  def generate(self):
    ...
    Fitting, generation and evaluation of R-MAT graphs is optional.
    """
    ...
    if self.config.generate.is_eval:
      ...
      # Compared with Test Set
      num_nodes_test = [len(gg.nodes) for gg in self.graphs_test]  # shape B X 1
      mmd_degree_test, mmd_clustering_test, mmd_4orbits_test, mmd_spectral_test, mmd_radius_test, mmd_diffusion_test_durs, mmd_diffusion_test_sats = evaluate(
      ...
      logger.info("\nValidation MMD scores of"
                  ...
                  "\nradius        {}"
                  "\ncommunity     {}".format(mmd_num_nodes_dev, mmd_degree_dev, mmd_clustering_dev, mmd_4orbits_dev,
                                              mmd_spectral_dev, mmd_diffusion_dev_durs, mmd_diffusion_dev_sats,
                                              mmd_radius_dev, mmd_num_coms_dev))
      ...
      logger.info("\nTest MMD scores of"
                  ...
                  "\nSIR sats.     {}"
                  "\nradius        {}"
                  "\ncommunity     {}".format(mmd_num_nodes_test, mmd_degree_test, mmd_clustering_test,
                                              mmd_4orbits_test,
